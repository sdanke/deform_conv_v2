...
#include "modulated_deform_im2col_cuda.cuh"
    ...
    const int64_t im2col_step)
{
    ...
    const int64_t channels_out = weight.size(0);
    const int64_t channels_kernel = weight.size(1);
    const int64_t kernel_h_ = weight.size(2);
    const int64_t kernel_w_ = weight.size(3);
    ...
    const int64_t height_out = (height + 2 * pad_h - (dilation_h * (kernel_h - 1) + 1)) / stride_h + 1;
    const int64_t width_out = (width + 2 * pad_w - (dilation_w * (kernel_w - 1) + 1)) / stride_w + 1;
    ...
    const int64_t per_input_size = channels * height * width;
    const int64_t per_offset_size = offset.size(1) * offset.size(2) * offset.size(3);
    const int64_t per_mask_size = mask.size(1) * mask.size(2) * mask.size(3);
    ...
    const int64_t im2col_step)
{
    ...
    const int64_t batch_ = grad_output.size(0);
    const int64_t channels_out_ = grad_output.size(1);
    const int64_t height_out_ = grad_output.size(2);
    const int64_t width_out_ = grad_output.size(3);

    const int64_t im2col_step_ = std::min(im2col_step, batch);
